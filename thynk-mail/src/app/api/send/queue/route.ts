...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (cErr || !campaign) {
      return NextResponse.json({ error: `Campaign not found: ${cErr?.message ?? 'no data'}` }, { status: 404 });
    }
    if (!campaign.account_id) {
      return NextResponse.json({ error: 'No sending account assigned' }, { status: 400 });
    }
    ...
    if (aErr || !account) {
      return NextResponse.json({ error: `Email account not found: ${aErr?.message ?? 'no data'}` }, { status: 404 });
    }
    ...
    if (clErr) {
      await db.from('campaigns').update({ status: 'failed' }).eq('id', campaign_id);
      return NextResponse.json({ error: `Failed to load contact lists: ${clErr.message}` }, { status: 500 });
    }
    ...
    if (contactsErr) {
      await db.from('campaigns').update({ status: 'failed' }).eq('id', campaign_id);
      return NextResponse.json({ error: `Failed to load contacts: ${contactsErr.message}` }, { status: 500 });
    }

    // Filter suppressions — graceful: skip if team_id missing or table errors
    ...
    if ((contacts ?? []).length > 0 && campaign.team_id) {
      ...
      suppressedSet = new Set((suppressions ?? []).map(s => s.email));
    }
    ...
    if (logsErr) {
      await db.from('campaigns').update({ status: 'failed' }).eq('id', campaign_id);
      return NextResponse.json({ error: `Failed to create send logs: ${logsErr.message}` }, { status: 500 });
    }
    ...
    let sentCount = 0;
    let failCount = 0;
    let currentSentToday = account.sent_today ?? 0;
    ...
    // Catch-all: any unhandled throw returns JSON instead of Next.js HTML error page
    const message = err instanceof Error ? err.message : String(err);
    console.error('[send/queue] Unhandled error:', message);

    // Best-effort: mark campaign failed so it's not stuck in 'sending' state
    try {
      const db = createServerClient();
      await db.from('campaigns').update({ status: 'failed' }).eq('id', campaign_id);
    } catch { /* ignore */ }
